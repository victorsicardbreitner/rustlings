// errors6.rs
//
// Using catch-all error types like `Box<dyn error::Error>` isn't recommended for library code, where callers might want to make decisions based on the
...
fn parse_pos_nonzero(s: &str) -> Result<PositiveNonzeroInteger, ParsePosNonzeroError> {
    // TODO: change this to return an appropriate error instead of panickin when `parse()` returns an error.
    

    if s.parse::<i64>().is_err() {
        Err(ParsePosNonzeroError::from_parseint(s.parse::<i64>().err().unwrap()))
    }
    else{
